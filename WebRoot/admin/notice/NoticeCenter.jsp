...
<html>
  ...
  function delFile(){
	  var dels=$("input[id='chBox']:checked");
	   	...
	   	$("#deleteArticleForm").attr("action","delNotices.do");
  ...
  function pageTO(){
	  ...
	  window.location.href="NoticeCenterPage.do?pageNum="+$pageNO;
  }
...
<%@ include file="../head.jsp" %>
 ...
 <ul class="nav nav-pills nav-stacked" style="margin-left: 6px">
  ...
  <div class="container" style="float:right ;width:76%;">   
   ...
   <tbody>
     ...
     <c:forEach items="${p.list}" var="page" >
         ...
         <td><a href="showNotice.do?id=${page.id}">${page.title}</a></td>
            
         <td>
         	 <fmt:formatDate value="${page.time}" pattern="yyyy年MM月dd日 "/>            
         </td>
          <td>${page.author}</td>  
         
         <td>
         	<c:if test="${page.open==true}">公布</c:if>
         	<c:if test="${page.open==false}">未公布</c:if>
         </td>
         <td>
  ...
  <li><a href="NoticeCenterPage.do?pageNum=${p.pageNo-1}">上一页</a></li>
  </c:if>
  <c:if test="${(p.pageNo-2)>=1}">
  	<li><a href="NoticeCenterPage.do?pageNum=${p.pageNo-2}">${p.pageNo-2}</a></li>
  </c:if> 
  
 <c:if test="${(p.pageNo-1)>=1}">
  	<li><a href="NoticeCenterPage.do?pageNum=${p.pageNo-1}">${p.pageNo-1}</a></li>
  </c:if>   
  	<li><a href="NoticeCenterPage.do?pageNum=${p.pageNo}">${p.pageNo}</a></li>
  ...
  <li><a href="NoticeCenterPage.do?pageNum=${p.pageNo+1}">下一页</a></li>&nbsp;&nbsp;&nbsp;&nbsp;
  </c:if>
   <c:if test="${p.pageNo+1>p.totalPage}">
  <li><a href="#">下一页</a></li>&nbsp;&nbsp;&nbsp;&nbsp;
  </c:if>
...
<c:if test="${p==null }">
<div align="center" > 
  <p style="font-size: 20px;">最近无公告</p>
</div>
</c:if>
   </form>
    <ul class="nav nav-pills nav-stacked" style="margin-left: 6px">
    <li class="active" style="text-align: center;"><a href="toAddNoticePage.do"><span class=" glyphicon glyphicon-pencil">&nbsp;发布公告</span></a></li>
    </ul>
   </div>
   </div>
   </div>
