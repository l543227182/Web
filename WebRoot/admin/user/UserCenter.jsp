...
<html>
  ...
  function delUser(){
	  var dels=$("input[id='chBox']:checked");
	   	...
	   	$("#deleteFileForm").attr("action","delUser.do");
	   	$("#deleteFileForm").attr("method","post").submit(); 
  } 
  function ModifyRoles(id){
	  window.location.href="ToModifyRolesPage.do?id="+id;
  }
...
<div class="panel panel-default"  style="float: left;width: 24% ;height: 95% ;border: 0px;">
  ...
  <lc:if PID="${user.PID}" value="16">
   ...
   <li  class="active"><a href="../user/UserManager.do">用户管理</a></li>
   </lc:if>
   </ul>
   </div>
  ...
  <div class="container" style="float:right ;width:76%;">
 	...
 	<div class="panel panel-primary" >  
 	 <div class="panel-heading">    
 	  <h3 class="panel-title">用户管理中心</h3>
 	     </div> 
 		...
 		<form id="deleteFileForm">
		   <table class="table table-hover">
	        ...
	        <button class="btn btn-default"  onclick="searchButton()">用户搜索</button>
	        </div>
		   ...
		   <thead>
		      <tr>
		      	...
		      	<a href="javascript:delUser()"> 删除</a></th>
		         <th>用户名</th>	
		         <th>真实姓名</th>	         
		         <th>e-mail</th>
		         <th>权限</th>
   ...
   <c:if test="${p!=null}">
     ...
     <c:forEach items="${p.list}" var="page" >
     	...
     	<tr class="success">
          <td> 
           <input type="checkbox" id="chBox" name="ids" value="${page.id}">   
      	 </td>
         <td><a href="toModifFilePage.do?FileID=${page.id}">${page.username}</a></td>
         <td>${page.realname}</td>       
       	 <td>${page.email}</td>
       	 <td>${page.NAME}</td>
         <td><fmt:formatDate value="${page.regDate}" pattern="yyyy年MM月dd日 "/></td>      
         <td>
         <c:if test="${page.ROLEID!='2'}">
         	<button type="button" class="btn btn-primary btn-sm" onclick="ModifyRoles('${page.id}')" >修改权限 </button>
        </c:if>
         </td>
      </tr>
     </c:forEach>
